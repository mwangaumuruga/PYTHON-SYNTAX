!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Inheritance and Modules in Python</title>
    <style>
        body {
            font-family: Arial, sans-serif;
            line-height: 1.6;
            margin: 20px;
            background-color: #fafafa;
        }
        h1 {
            color: red;
            text-align: center;
            margin-bottom: 20px;
        }
        h2 {
            color: darkgreen;
            margin-top: 30px;
        }
        h3 {
            color: blue;
            margin-top: 20px;
        }
        p {
            font-size: 16px;
        }
        pre {
            background-color: #f4f4f4;
            padding: 12px;
            border-radius: 8px;
            overflow-x: auto;
            font-family: Consolas, monospace;
            font-size: 15px;
        }
        code {
            color: #c7254e;
        }
        hr {
            margin: 20px 0;
        }
    </style>
</head>
<body>
    <h1>Modules in Python</h1>

 
    <h2>🐾 MODULES in Python</h2>
    <p>

"""  are usually imported and generally they are a set of prewrittten code that helps reduce the work of 
the work of rewriting codes that can be used i.e Maths standards  or a code generate randomness and this helps save 
time and also get us on pint without focusing on the complex stuff"""
example use case THE RANDOM MODULE
# A program to randomly  choose a country from a list of must visit countries
import random
countries=("rwanda","mauritius","seychelles","southafrica","madagascar""tunisia","turkey","israel","usa","china","spain")
print(random.choice(countries))
<p>
</body>
</html>

